...
public sealed class SchedulingSettings
{
    ...
    public void Update(int slotIntervalMinutes, int bufferBeforeMinutes, int bufferAfterMinutes, string timeZoneId) { if (slotIntervalMinutes <= 0 || slotIntervalMinutes > 120 || bufferBeforeMinutes < 0 || bufferAfterMinutes < 0 || string.IsNullOrWhiteSpace(timeZoneId)) throw new ArgumentException("Scheduling settings are invalid."); SlotIntervalMinutes = slotIntervalMinutes; BufferBeforeMinutes = bufferBeforeMinutes; BufferAfterMinutes = bufferAfterMinutes; TimeZoneId = timeZoneId.Trim(); }
}
